...
def generate_totp() -> Tuple[str, int]:
    """
    Returns (code, valid_for_seconds) using the seed from ./data/seed.txt
    """
    seed_path = os.path.join('.', 'data', 'seed.txt')
    if not os.path.isfile(seed_path):
        raise FileNotFoundError("Seed file not found at ./data/seed.txt")
    with open(seed_path, 'r') as f:
        seed = f.read().strip()
    return current_code_and_remaining(seed)
